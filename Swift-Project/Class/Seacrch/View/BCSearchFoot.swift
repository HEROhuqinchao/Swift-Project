...
class BCSecarchCell: BCBaseCollectionViewCell {
...
protocol BCSearchFootDelegate: class {
    func searchTFoot(_ searchTFoot: BCSearchFoot, didSelectItemAt index: Int, _ model: SearchItemModel)
}

class BCSearchFoot: BCBaseTableViewHeaderFooterView {

    weak var delegate: BCSearchFootDelegate?
    lazy var collectionView: UICollectionView = {
        let lt = BCCollectionViewAlignedFlowLayout()
        ...
        cw.dataSource = self
        cw.delegate = self
        cw.register(cellType: BCSecarchCell.self)
        return cw
    }()
...
extension BCSearchFoot: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: BCSecarchCell.self)
