...
public enum BCNoticeBarStyle {
    case onStatusBar
    case onNavigationBar
}

public enum BCNoticeBarAnimationType {
    case top
    case bottom
    case left
    case right
...
extension BCNoticeBarAnimationType {
    
    fileprivate func noticeBarViewTransform(with frame: CGRect,  _ style: BCNoticeBarStyle) -> CGAffineTransform {
...
extension BCNoticeBarStyle {
    
   fileprivate func noticeBarProperties() -> BCNoticeBarProperties {
    let screenWidth = UIScreen.main.bounds.width
    var properties: BCNoticeBarProperties
    switch self {
    case .onNavigationBar:
        properties = BCNoticeBarProperties(shadowOffsetY: 3,
                                          ...
                                          viewFrame: CGRect(origin: CGPoint.zero,
                                                            size: CGSize(width: screenWidth,
                                                                         height: 44.0 + (UIApplication.shared.windows.first { $0.isKeyWindow }?.windowScene?.statusBarManager?.statusBarFrame.height)! )))
    case .onStatusBar:
        properties = BCNoticeBarProperties(shadowOffsetY: 2,
                                          fontSizeScaleFactor: 0.75,
                                          textFont: UIFont.systemFont(ofSize: 13),
                                          viewFrame: CGRect(origin: CGPoint.zero,
                                                            size: CGSize(width: screenWidth,
                                                                         height: UIApplication.shared.windows.first { $0.isKeyWindow }?.windowScene?.statusBarManager?.statusBarFrame.height ?? 0)))
          }
          return properties
    }
    ...
    fileprivate func noticeBarOriginY(superViewHeight: CGFloat, _ height: CGFloat) -> CGFloat {
        ...
        switch self {
        case .onNavigationBar:
            let height = UIApplication.shared.windows.first { $0.isKeyWindow }?.windowScene?.statusBarManager?.statusBarFrame.height ?? 0
            originY = height + (superViewHeight - height - height) * 0.5
        case .onStatusBar:
...
fileprivate struct BCNoticeBarProperties {
...
public struct BCNoticeBarConfig {
    public init() {}
    ...
    public var animationType = BCNoticeBarAnimationType.top
    public var barStyle = BCNoticeBarStyle.onNavigationBar
    
    public init(title: String? = nil,
                image: UIImage? = nil,
                textColor: UIColor = UIColor.white,
                backgroundColor: UIColor = UIColor.orange,
                barStyle: BCNoticeBarStyle = .onNavigationBar,
                animationType: BCNoticeBarAnimationType = .top) {
        self.title = title
        self.image = image
...
open class BCNoticeBar: UIView {
    
    private var config = BCNoticeBarConfig()
    ...
    public func show(duration: TimeInterval, completed: ((_ finished: Bool) -> Void)? = nil) {
        self.show(duration: duration, willShow: {
            ...
            UIApplication.shared.windows[0].windowLevel = currentWindowLevel
            }, completed: {
    ...
    public init(config: BCNoticeBarConfig) {
    ...
    private func show(duration: TimeInterval, willShow: () -> Void, completed: ((_ finished: Bool) -> Void)?) {
        ...
        if let subviews = UIApplication.shared.windows.first(where: { $0.isKeyWindow })?.subviews {
            for view in subviews {
                if view.isKind(of: BCNoticeBar.self) {
